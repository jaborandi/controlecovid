	...
	<?php 
		if($status==true){
			if(!empty($_GET['resend'])){
				?>
				<h4 class="text-info bold animated bounce"><i class="material-icons">email</i> A confirmação por e-mail foi reenviada</h4>
				<?php
			}
			else{
				?>
				<h4 class="text-info bold"><i class="material-icons">email</i> Link de confirmação de e-mail enviado</h4>
				<?php
			}
		?>
			...
			?>
			<div><i class="material-icons">email</i> Por favor, verifique seu endereço de e-mail seguindo o link em sua caixa postal</div>
			<hr />
			<div>
				<a href="<?php print_link("index/send_verify_email_link/$user_email?resend=true") ?>" class="btn btn-primary"><i class="material-icons">email</i> Reenviar email</a>
			</div>
