...
class UsuarioController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				usuario.id LIKE ? OR 
				usuario.login LIKE ? OR 
				usuario.senha LIKE ? OR 
				usuario.email LIKE ? OR 
				usuario.foto LIKE ?
			)";
			$search_params = array(
		...
		if(!empty($request->orderby)){
			...
			$db->orderBy("usuario.id", ORDER_TYPE);
		}
		if($fieldname){
		...
		$page_title = $this->view->page_title = "Usuario";
		$this->view->report_filename = date('Y-m-d') . '-' . $page_title;
		$this->view->report_title = $page_title;
		$this->view->report_layout = "report_layout.php";
		$this->view->report_paper_size = "A4";
		$this->view->report_orientation = "portrait";
		$view_name = (is_ajax() ? "usuario/ajax-list.php" : "usuario/list.php");
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("usuario.id", $rec_id);; //select record based on primary key
		}
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("login","senha","email","foto");
	...
	function edit($rec_id = null, $formdata = null){
		...
		$page_title = $this->view->page_title = "Editar Usuario";
		if(!$data){
			$this->set_page_error();
		}
	...
	function delete($rec_id = null){
		...
		$db->where("usuario.id", $arr_rec_id, "in");
